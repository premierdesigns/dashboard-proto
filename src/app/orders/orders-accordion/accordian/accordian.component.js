"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var core_1 = require("@angular/core");
require("rxjs/add/operator/map");
var AccordianComponent = (function () {
    function AccordianComponent() {
        this.activeOrderId = 0;
    }
    AccordianComponent.prototype.toggle = function (order) {
        this.isActive(order) ? this.activeOrderId = 0 : this.activeOrderId = order.id;
    };
    AccordianComponent.prototype.isActive = function (order) {
        return order.id === this.activeOrderId;
    };
    AccordianComponent.prototype.ngOnInit = function () {
    };
    return AccordianComponent;
}());
__decorate([
    core_1.Input()
], AccordianComponent.prototype, "orders", void 0);
__decorate([
    core_1.Input()
], AccordianComponent.prototype, "filterOrders", void 0);
AccordianComponent = __decorate([
    core_1.Component({
        selector: 'pd-accordian',
        templateUrl: './accordian.component.html',
        styleUrls: ['./accordian.component.less']
    })
], AccordianComponent);
exports.AccordianComponent = AccordianComponent;
